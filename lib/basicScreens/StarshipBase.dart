...
class StarshipBase extends StatefulWidget {
  final String id;

  StarshipBase({required this.id});


  @override
  _StarshipBaseState createState() => _StarshipBaseState();
}

class _StarshipBaseState extends State<StarshipBase> {
  ...
  Future<void> _fetchFilmData() async {
    ...
    try {
      final response = await http.get(Uri.parse('http://192.168.0.78:3000/starships/ ${widget.id}'),headers: {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          hyperdrive_rating = jsonData['data_json']['hyperdrive_rating'];
          MGLT = jsonData['data_json']['MGLT'];
          starship_class = jsonData['data_json']['starship_class'];
        });
      } else {
          ...
          child: _isLoading
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 20),
                Text('Hyperdrive rating: $hyperdrive_rating'),
                SizedBox(height: 20),
                Text('MGLT: $MGLT'),
                SizedBox(height: 20),
                Text('Starship class: $starship_class'),
              ],
            ),
          )
      ),
